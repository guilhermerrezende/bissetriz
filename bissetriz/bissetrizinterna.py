def calcular_razao_bissetriz_interna(lado_ab, lado_ac, lado_bc):
...
try:
    ...
    resultado = calcular_razao_bissetriz_interna(lado_ab, lado_ac, lado_bc)
    if isinstance(resultado, str):
        print(resultado)  
    else:
        bd, dc, razao = resultado
        print(f"Segmentos BD e DC são, respectivamente: {bd:.2f} e {dc:.2f}")
        print(f"A razão BD/DC é: {razao:.2f}, confirmando a proporção AB/AC.")
except ValueError:
